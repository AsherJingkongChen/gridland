...
export const updateChunks = async () => {
  let cx = Math.floor(camera.x / PixelPerChunk);
  let cy = Math.floor(camera.y / PixelPerChunk);
  ...
  Db.transaction(
    'readwrite',
    [ Db.chunks ],
    async () => { return updateChunksAt(cx, cy); }
  );
};

export const updateChunksAt =
